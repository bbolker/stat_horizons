...
```{r mod_list}
mod_list <- tibble::lst(m_moritz, m_full, m_fullmax, m_buildmer@model, m_buildmer_fix@model)
```
...
```{r diag1}
performance::check_model(m_buildmer_fix@model)
pp1 <- DHARMa::simulateResiduals(m_builder_fix@model)
pp2 <- DHARMa::simulateResiduals(m_builder_fix@model, use.u = TRUE)
plot(pp2)
```

```{r diag2}
plot(simulateResiduals(m_buildmer_fix@model))
performance::check_model(m_buildmer_fix@model)
```
...
```{r effects}
plot(allEffects(m_builder_fix@model))
```
