...
m1 <- lmer(Reaction ~ Days + (Days|Subject), ss)
car::Anova(m1)
```

```{r glmmTMB_fit, echo = FALSE}
m2 <- glmmTMB(Reaction ~ Days + (Days|Subject), ss, REML = TRUE)
```
...
* switch a random effect to fixed
* simplify the model
...
* @scandolaReliability2024a
* Suppose factor `f` (5 levels) varies within groups `g`
* `(f|g)`: $(n(n+1))/2 = (5\times 6)/2 = 15$ parameters)
...
## Reduced-rank covariance models
...
# Challenges & open questions {.section}
