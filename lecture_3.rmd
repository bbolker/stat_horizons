...
the *expected* among-group variation $\left( = \frac{\sigma^2_{\textrm{among}} +
\sigma^2_{\textrm{within}}}{n} \right)$. More generally, because *some* dimension
...
* `blme`, `glmmTMB` + prior specification
* regularization useful in other cases (e.g. complete separation in logistic models)
...
* unidentifiable for binary data (unless groupable), Gaussian data
* little concern for families with estimated scale/dispersion (but see @hilbe_overdispersion_2011)
...
* can be an issue (especially among groups)
* log transformation (for continuous positive data) often resolves both linearity and heteroscedasticity
* `glmmTMB`, `nlme`
* build into observation-level REs
