...
```{r ggplot0, warning = FALSE, message = FALSE}
...
Decides on intercepts only at each stage. Unfortunately it also does model selection on the fixed effects by default, which I **strongly** disagree with (for philosophy about model selection/averaging see @bolkerMultimodel2024a)
...
In @moritzrole2023a, we fitted all combinations of [intercept-only, diagonal, full] at each level [biome, realm, biome × realm]: 3 choices at each of three hierarchical levels → 81 sub-models to fit. We chose the *minimum-AIC*, *non-singular* model, which turned out to be a diagonal covariance matrix at the (biome×realm) level and intercepts only at the other two levels:
...
```{r ranef, fig.width = 12, fig.height = 10}
