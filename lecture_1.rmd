...
* use fixed effects (or *two-stage models*) instead of random effects when there are
     * many observations per block (shrinkage unimportant)
...
- random effects specified with `|` ("pipe"/"bar") as `(a|g1) + (b|g2) + ...`
...
```{r plotcov2}
fit2 <- simfun(c(10, 1), c(2,1), 3,
       y ~ 1 + (v|g),
       list(beta = 1, theta = c(2,2,2), sigma = 1))
plotfun(fit2)
```
...
## The maximal model is often too complex

- for a RE where $n$ effects vary across clusters we need to estimate $n(n+1)/2$ covariance parameters
- e.g. bloodworm example: 5×4×4×2 factorial design (160 combinations), all measured in each of 10 individuals. 
    - Even treating covariates as numeric, allowing all interactions, gives 16 effects → 136 parameters
    - Categorical predictors, without interactions: 12 effects → 78 parameters
    - Numeric predictors, no interactions: 4 effects → 10 parameters
- **but** ignoring variation of within-cluster effects can also be dangerous [@schielzeth_conclusions_2009; heisigWhy2019a; @oberauerImportance2022]
...
## 'Old' and 'new' random effects 

* @hodges_richly_2016
* are random effects/latent variables modeling exchangeable groups ...
