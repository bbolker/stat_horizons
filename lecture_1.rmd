...
set.seed(20251028)
## Okabe-Ito minus black, yellow
OkIt <- unname(palette.colors(n = 8, palette = "Okabe-Ito"))[-c(1,5)]
...
* shoulders of giants
* especially
   * Doug Bates, Martin Mächler, Steve Walker (`lme4`)
   * Kasper Kristensen, Mollie Brooks (`glmmTMB`)
* the R community, `r-sig-mixed-models` denizens, bug reporters ...
* McMaster, NSERC, Univ Florida, US NSF, BIRS
...
* multiple grouping variables (random effects *terms*); multiple effects per group ("*random slopes*")
* interesting *conditional distributions*
...
* structured covariances: autoregressive, compound symmetry, regression splines ...
...
## Wildfire/diversity [@moritzrole2023a]

```{r fire-fig, echo = FALSE, out.height = "100%"}
...
& = (\beta_0 + \epsilon_{1,j}) + \beta_1 x_{ij} + \epsilon_{0,ij} \\
...
- correlated variation in intercepts/slopes

## Terminology

- **grouping variables** define sets of clusters (e.g. school, year, country ...)
- **varying effects**: predictors whose effects vary across clusters (e.g. intercept, herbivory, time)
- random effects **term**: the combination of a clustering variable and a set of varying effects
- **conditional mode** ("BLUP"): the predicted value of the *difference* of a cluster effect from the population mean
- **conditional distribution**: distribution of the data (Gaussian, Poisson, binomial, Beta, Tweedie, ...)
...
- have more than <footnote content="Economists say 30-50">5-10 levels</footnote> [@clarkeWhenCanGroup2008; @austinEffectNumberClusters2018]
...
* use fixed effects (or *two-stage models*) instead of random effects when there are
     ...
     * few clusters (little advantage in # parameters; bad variance estimates)
...
- Crossed+: `(1|f*g)` $\equiv$ `(1|f) + (1|g) + (1|f:g)`. Years vary, plots vary independently, plots vary within years (need >1 observation per plot/year combination, for LMMs).
...
* 5 groups, 2 effects (v1) × 5 groups, 1 effect (v2)
...
@moritzrole2023a (fire/diversity; blocks are geographic regions)
...
- **deterministic**
	...
	- need best methods for small clusters (binary, small # of observations)
	- available methods depend on problem structure
    - accuracy (GHQ) vs. flexibility and speed (PQL, Laplace)
- **stochastic** (Monte Carlo): mostly Bayesian, some frequentist
...
tt |> filter(is.na(effect) | effect == "fixed") |>
  ...
  geom_pointrange(aes(xmin = conf.low, xmax = conf.high),
                  position = position_dodge(width = 0.35, reverse = TRUE)) +
